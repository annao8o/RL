...
class QlearningPolicy(Policy):
    ...
    def act(self, state, eps=0.1):
        ...
        max_q = max(qvals.values())     ### cost로 하려면 min을 pick 해야 할 듯
    ...
    def _update_q_value(self, tr, alpha):
        ...
        # do not include the value of the next state if terminated.
    ...
    class TrainConfig(TrainConfig):
        ...
        def train(self, config: TrainConfig):
            ...
            for n_episode in range(config.n_episodes):
                #initalize at start of every episode
                ob = self.env.reset()
                done = False
                reward = 0.

                while not done:
                    a = self.act(ob, eps)   #pick best action
                    new_ob, r, done, info = self.env.step(a)
                    if done and config.don_reward is not None:
                ...
                reward_averaged.append(np.average(reward_history[-50:]))    #check why -50?
